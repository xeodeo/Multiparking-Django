...
# -*- coding: utf-8 -*-
"""
Convierte los 3 manuales de usuario (Markdown) a .docx, embebiendo las
imágenes reales e incluyendo tablas, negritas, listas y encabezados.

Uso: python build_docx_manuales.py
Genera manual-<rol>.docx junto a cada manual-<rol>.md.
"""
import os
import re
import sys
...
def add_inline_runs(paragraph, text, base_size=10.5, base_color=COLOR_BODY):
    """Parses **bold**, *italic*, `code`, and [text](url) inside a line."""
    pos = 0
...
def add_heading(doc, text, level):
    ...
    add_inline_runs.__wrapped__ = None
    run = p.add_run(text)
...
def add_paragraph_text(doc, text, size=10.5):
    p = doc.add_paragraph()
...
def add_image(doc, base_dir, rel_path, caption=None):
    ...
    # thin border/frame around image via a following faint caption spacing
    if caption:
...
def convert(md_path, docx_path):
    ...
    while i < n:
        ...
        # Regular paragraph: accumulate until blank line or a new block starts
        buf = [stripped]
        i += 1
        while i < n and lines[i].strip() and not re.match(r'^(#{1,6}\s|!\[|\||>|---\s*$|\s*[-*]\s|\s*\d+\.\s)', lines[i]):
            buf.append(lines[i].strip())
            i += 1
        add_paragraph_text(doc, ' '.join(buf))
...
if __name__ == '__main__':
    ...
    targets = [
        ('manual/administrador/manual-administrador.md', 'manual/administrador/manual-administrador.docx'),
        ('manual/vigilante/manual-vigilante.md', 'manual/vigilante/manual-vigilante.docx'),
        ('manual/cliente/manual-cliente.md', 'manual/cliente/manual-cliente.docx'),
    ]
    for md_rel, docx_rel in targets:
        convert(os.path.join(root, md_rel), os.path.join(root, docx_rel))
