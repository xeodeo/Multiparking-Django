...
## Corpus Check
- 234 files · ~1,021,016 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1329 nodes · 2187 edges · 117 communities (88 shown, 29 thin omitted)
...
- `CuponeraClienteView` --uses--> `Cupon`  [INFERRED]
  fidelidad/views.py → cupones/models.py
- `FidelidadConfigView` --uses--> `Cupon`  [INFERRED]
  fidelidad/views.py → cupones/models.py
- `PerfilClienteView` --uses--> `Cupon`  [INFERRED]
  fidelidad/views.py → cupones/models.py
- `ReclamarBonoView` --uses--> `Cupon`  [INFERRED]
  fidelidad/views.py → cupones/models.py
...
Cohesion: 0.06
Nodes (67): CuponAplicado, EspacioAdmin, InventarioParqueoAdmin, PisoAdmin, register, TipoEspacioAdmin, Espacio, EstadoChoices (+59 more)

### Community 1 - "Usuario"
Cohesion: 0.05
Nodes (40): CuponAdmin, CuponAplicadoAdmin, register, Cupon, Meta, Calcula el monto a descontar según el tipo del cupón. Centraliza la lógica…, TipoChoices, cuponera_context() (+32 more)
...
### Community 5 - "usuarios/views.py"
Cohesion: 0.09
Nodes (23): never_cache, _is_ajax(), Valida los campos comunes de usuario. Retorna (True, None) si válido; (False,…, validar_datos_usuario(), AdminTestEmailView, dashboard_view(), home_view(), login_view() (+15 more)
...
### Community 8 - "cupones/views.py"
Cohesion: 0.17
Nodes (8): CuponCreateView, CuponDeleteView, CuponListView, CuponUpdateView, View, Validators compartidos entre apps. Centraliza funciones de validación para…, Parsea y valida un valor decimal positivo. Retorna (Decimal, None) si es…, validar_decimal_positivo()
...
### Community 13 - "fidelidad/views.py"
Cohesion: 0.13
Nodes (16): ConfiguracionFidelidad, Meta, Un sticker ganado por un usuario al estar parqueado más de 1 hora., Configuración global del sistema de stickers (singleton)., Sticker, _build_usuarios_log(), CuponeraClienteView, FidelidadConfigView (+8 more)

### Community 14 - "email_utils.py"
Cohesion: 0.15
Nodes (23): _build_msg(), enviar_bienvenida(), enviar_bono_stickers(), enviar_confirmacion_entrada(), enviar_confirmacion_reserva(), enviar_novedad(), enviar_recibo_pago(), enviar_recordatorio_reserva() (+15 more)
...
### Community 16 - "Reserva"
Cohesion: 0.16
Nodes (8): register, ReservaAdmin, Vistas para que los clientes creen y gestionen sus reservas, EstadoChoices, Meta, Cambia el estado de la reserva y libera el espacio si estaba RESERVADO., Cancela las reservas PENDIENTES cuya hora de inicio ya pasó o está en ≤15 min,…, Reserva

### Community 17 - "reservas/views.py"
Cohesion: 0.14
Nodes (14): AdminClientesVehiculosAPIView, AdminCrearReservaView, View, Admin crea una reserva desde el mapa de espacios del dashboard., Devuelve clientes activos y sus vehículos para el modal de reserva., API endpoint para obtener detalles de una reserva, ReservaCancelarView, ReservaDetallesAPIView (+6 more)
...
### Community 20 - "reportes_views.py"
Cohesion: 0.27
Nodes (6): ExportarExcelReportesView, ExportarPDFReportesView, View, Vistas para el módulo de Reportes y Estadísticas, Vista principal de Reportes y Estadísticas con gráficas interactivas, ReportesView
...
### Community 23 - "build_docx_despliegue.py"
Cohesion: 0.09
Nodes (27): add_para(), body(), cover_line(), h1(), h2(), _mark_header_row(), _set_cell_borders(), _set_cell_margins() (+19 more)
...
### Community 28 - "enviar_recordatorios.py"
Cohesion: 0.40
Nodes (3): Command, BaseCommand, Management command para enviar recordatorios de reservas 24 horas antes. Uso:…
...
### Community 31 - "mixins.py"
Cohesion: 0.06
Nodes (31): PagosListView, View, Vista para listar pagos con estadísticas, búsqueda y filtros., ReciboView, ClienteCancelarReservaView, ClienteConfirmarReservaView, ClienteCrearReservaView, ClienteEditarReservaView (+23 more)
...
- **Why does `AdminRequiredMixin` connect `multiparking/urls.py` to `Usuario`, `usuarios/views.py`, `cupones/views.py`, `fidelidad/views.py`, `reservas/views.py`, `reportes_views.py`, `novedades/views.py`, `mixins.py`?**
  _High betweenness centrality (0.018) - this node is a cross-community bridge._
- **Why does `Espacio` connect `multiparking/urls.py` to `Usuario`, `Reserva`, `reservas/views.py`, `reportes_views.py`, `novedades/views.py`?**
...
- **Are the 35 inferred relationships involving `CuponAplicado` (e.g. with `CuponAdmin` and `CuponAplicadoAdmin`) actually correct?**
  _`CuponAplicado` has 35 INFERRED edges - model-reasoned connections that need verification._